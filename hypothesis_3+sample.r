...
# Test if there is evidence that the diameter of the shells of abalones is different 
# for the three different sexes
...
levene.test(abalone[,3], abalone[,1])

# theoretically from the moment we rejected normality we can use the Kruskal Wallis test, 
# But since ANOVA is considered robust to violation of one of the two assumptions 
...
# Check the CI for each pair
t.test(abalone[abalone[,1]=="M",3], abalone[abalone[,1]=="F",3])
t.test(abalone[abalone[,1]=="M",3], abalone[abalone[,1]=="I",3])
t.test(abalone[abalone[,1]=="F",3], abalone[abalone[,1]=="I",3])
...
# test whether the diameter is different for the three different sexes and for abalones 
# that have small or large length

# Here I want to create a second categorical variable and therefore I split 
# the abalones to those with length >0.5 (large abalones) and
...
# test if the proportion of male, female and infant abalones with diameter greater than 0.25 
# are equal. 
table(abalone[,1])
...
# is there evidence that abalone sex and binary length are independent 
...
# change the number of bins to 5 - notice how probabilities changes
wholeweight<-abalone[,5]
probs5 <- rep(0.2, 5)
...
# Check if data fit chi square distribution using 4 cells. 
wholeweight<-abalone[,5]
probs4 <- rep(0.25, 4)
quan <- qchisq(c(0.25, 0.5, 0.75), df= mean(wholeweight))
obs <- c(length(wholeweight[wholeweight<quan[1]]), length(wholeweight[wholeweight<quan[2]]) -
            length(wholeweight[wholeweight<quan[1]]), length(wholeweight[wholeweight<quan[3]]) -
            length(wholeweight[wholeweight<quan[2]]), length(wholeweight) -
            length(wholeweight[wholeweight<quan[3]]))
chisq.test(obs, p=probs4)
pchisq(2316.4, 2, lower.tail=FALSE)
